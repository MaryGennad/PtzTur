...
document.querySelectorAll('img[alt="ПТЗ ТУР"]').forEach(img => {
    ...
    const paths = ['logoPTZTUR5.svg', '/logoPTZTUR5.svg', '/logoPTZTUR5.svg', '/img/logoPTZTUR5.svg']
        .filter(p => p !== current);
    let i = 0;
...
// ========== ЦЕЛИ ЯНДЕКС МЕТРИКИ ==========
const METRIKA_ID = 00000000; // ваш номер счётчика

// Цель 1: отправлена заявка (вставьте также внутрь submitBooking после validateBookingForm)
function sendMetrikaGoal(goalName) {
    if (typeof ym === 'function') ym(METRIKA_ID, 'reachGoal', goalName);
}

// Заявка успешно отправлена
const oldShowSuccess = showSuccessModal;
showSuccessModal = function () {
    sendMetrikaGoal('booking_submit');
    oldShowSuccess();
};

// Клики по телефону
document.querySelectorAll('a[href^="tel:"]').forEach(a => {
    a.addEventListener('click', () => sendMetrikaGoal('phone_click'));
});

// Клики по кнопкам "Забронировать"
document.querySelectorAll('.route-btn-primary, .cta-btn').forEach(btn => {
    btn.addEventListener('click', () => sendMetrikaGoal('booking_click'));
});
